    ...
    <div class="goods">
        ...
        <div class="menu-wrapper" ref="menuScroll">
            <ul>
                ...
                <li class="menu-item" :class="{'current':currentIndex === index + 1}" v-for="(item,index) in goods"
                    :key="index" @click="selectMenu(index+1)">
                    <p class="text">
        ...
        created() {
            fetch("/api/goods").then(res => {
                ...
                if (response.code === 0) {
                    ...
                    this.$nextTick(() => { //created钩子函数执行的时候,methods执行完了,但是DOM还未渲染
                        //执行滚动方法
                        this.initScroll();
                        //计算分类区间高度
            ...
            currentIndex() {
                for (let i = 0; i < this.listHeight.length; i++) {
                    ...
                    if (!height2 || (this.scrollY >= height1 && this.scrollY < height2)) {
                        // console.log(i); //根据滚动位置确认下标,与左侧对应
                        return i
                    }
                }
                return 0
